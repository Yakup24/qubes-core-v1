...
def mock_getaddrinfo(host, port, family=0, *args, **kwargs):
    if host == 'yum.qubes-os.org' and family == socket.AF_INET:
        return [(family, None, None, '', ('147.75.32.69', port))]
    if host == 'ripe.net' and family == socket.AF_INET6:
        return [(family, None, None, '', ('2001:67c:2e8:22::c100:68b', port))]
    raise socket.gaierror('mocked resolver has no entry for {}'.format(host))
...
class TestIptablesWorker(TestCase):
    def setUp(self):
        ...
        self.resolver_patch = patch('socket.getaddrinfo', mock_getaddrinfo)
    ...
    def test_002_prepare_rules4_ports_without_proto(self):
        rules = [{'action': 'accept', 'dstports': '443-443'}]
        expected_iptables = (
            "*filter\n"
            "-A chain -p tcp --dport 443:443 -j ACCEPT\n"
            "-A chain -p udp --dport 443:443 -j ACCEPT\n"
            "COMMIT\n"
        )
        self.assertEqual(self.obj.prepare_rules('chain', rules, 4),
            expected_iptables)
...
class TestFirewallWorker(TestCase):
    ...
    def test_read_rules_rejects_malformed_values(self):
        bad_rules = {
            '10.137.1.1': b'proto=tcp dstports=80=90 action=accept',
            '10.137.1.2': b'proto=tcp proto=udp action=accept',
            '10.137.1.3': b'proto=tcp dstports action=accept',
        }
        for addr, rule in bad_rules.items():
            self.obj.qdb.entries[
                '/qubes-firewall/{}/policy'.format(addr)] = b'drop'
            self.obj.qdb.entries[
                '/qubes-firewall/{}/0000'.format(addr)] = rule
            with self.assertRaises(qubesagent.firewall.RuleParseError):
                self.obj.read_rules(addr)

    def test_prepare_rules_rejects_unsafe_values(self):
        bad_rules = [
            {'action': 'accept', 'proto': 'tcp;drop'},
            {'action': 'accept', 'dst4': '1.2.3.4; drop'},
            {'action': 'accept', 'dstports': '65536'},
            {'action': 'accept', 'dstports': '90-80'},
            {'action': 'accept', 'proto': 'icmp', 'dstports': '8'},
            {'action': 'accept', 'dsthost': 'bad host'},
            {'action': 'accept', 'specialtarget': 'metadata'},
            {'action': 'accept', 'icmptype': '8;drop'},
            {'action': 'accept', 'proto': 'tcp', 'icmptype': '8'},
        ]
        for rule in bad_rules:
            with self.assertRaises(qubesagent.firewall.RuleParseError):
                IptablesWorker().prepare_rules('chain', [rule], 4)
            with self.assertRaises(qubesagent.firewall.RuleParseError):
                NftablesWorker().prepare_rules('chain', [rule], 4)

    def test_log_error_sets_display(self):
        with patch.dict('os.environ', {}, clear=True):
            self.obj.log_error('test message')
        self.subprocess_mock.assert_called_with(
            ['notify-send', '-t', '3000', 'test message'],
            env={'DISPLAY': ':0'})
