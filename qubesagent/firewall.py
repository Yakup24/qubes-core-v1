...
_SAFE_ICMP_TYPE_RE = re.compile(r'^[A-Za-z0-9_-]+$')
_SUPPORTED_PROTOCOLS = frozenset(('tcp', 'udp', 'icmp'))
    ...
    for elem in rule.split():
        try:
            ...
            raise RuleParseError(
                'Invalid rule element {!r}'.format(elem))
        if not key or not value or '=' in value:
            raise RuleParseError(
                'Invalid rule element {!r}'.format(elem))
        if key in rule_dict:
            raise RuleParseError(
                'Duplicate rule option {!r}'.format(key))
        rule_dict[key] = value
    return rule_dict


def _validate_addr(value, family):
    try:
        network = ipaddress.ip_network(value, strict=False)
    except ValueError as e:
        raise RuleParseError(
            'Invalid IPv{} address/network {!r}: {}'.format(
                family, value, e))
    if network.version != family:
        raise RuleParseError(
            'IPv{} address expected, got {!r}'.format(family, value))
    return value


def _validate_ip(value, family):
    try:
        addr = ipaddress.ip_address(value)
    except ValueError as e:
        raise RuleParseError(
            'Invalid IPv{} address {!r}: {}'.format(family, value, e))
...
def _validate_host(value):
    if not _SAFE_HOST_RE.match(value) or '..' in value:
        raise RuleParseError('Invalid hostname {!r}'.format(value))
    return value


def _validate_proto(value):
    if value not in _SUPPORTED_PROTOCOLS:
        raise RuleParseError('Unsupported protocol {!r}'.format(value))
    return value


def _validate_port(value):
    if not value.isdigit():
        raise RuleParseError('Invalid port {!r}'.format(value))
    port = int(value)
    if port < 1 or port > 65535:
        raise RuleParseError('Port out of range {!r}'.format(value))
    return port
...
def _validate_ports(value):
    parts = value.split('-')
    if len(parts) > 2:
        raise RuleParseError('Invalid port range {!r}'.format(value))
    first = _validate_port(parts[0])
    if len(parts) == 1:
        return value
    second = _validate_port(parts[1])
    if first > second:
        raise RuleParseError('Invalid descending port range {!r}'.format(value))
    return value


def _validate_icmptype(value):
    if not _SAFE_ICMP_TYPE_RE.match(value):
        raise RuleParseError('Invalid ICMP type {!r}'.format(value))
    return value


def _validate_rule(rule, family):
    if rule.get('action') not in ('accept', 'drop'):
        raise RuleParseError(
            'Invalid rule action {}'.format(rule.get('action')))
    if 'proto' in rule:
        _validate_proto(rule['proto'])
    if 'dst4' in rule:
        _validate_addr(rule['dst4'], 4)
    if 'dst6' in rule:
        _validate_addr(rule['dst6'], 6)
    if 'dsthost' in rule:
        _validate_host(rule['dsthost'])
    if 'dstports' in rule:
        _validate_ports(rule['dstports'])
        if 'proto' in rule and rule['proto'] not in ('tcp', 'udp'):
            raise RuleParseError(
                'Ports are supported only for tcp/udp rules')
    if 'specialtarget' in rule and rule['specialtarget'] != 'dns':
        raise RuleParseError(
            'Unsupported special target {!r}'.format(rule['specialtarget']))
    if 'icmptype' in rule:
        _validate_icmptype(rule['icmptype'])
        if rule.get('proto') != 'icmp':
            raise RuleParseError(
                'ICMP type is supported only for icmp rules')
...
class FirewallWorker(object):
    def __init__(self):
        if qubesdb is None:
            raise RuntimeError('qubesdb is required to run qubes-firewall')
...
def main():
    if daemon is None:
        raise SystemExit('python-daemon is required to run qubes-firewall')
    if shutil.which('nft'):
